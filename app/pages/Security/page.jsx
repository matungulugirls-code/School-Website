...
import { FiArrowLeft, FiLayers, FiX, FiChevronRight, FiShield } from 'react-icons/fi';
...
const SecurityCard = ({ item, onView }) => {
        ...
        <div className="relative h-44 overflow-hidden rounded-[22px] border border-white/60 bg-slate-100 shadow-sm">
          {imageUrl ? (
            ...
            <div className="h-full w-full flex items-center justify-center bg-gradient-to-br from-slate-50 to-slate-100">
              <FiShield className="text-slate-400" size={22} />
            </div>
          )}
...
export default function SecurityPage() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        setLoading(true);
        setError('');
        const res = await fetch('/api/schoolhub?type=SECURITY');
        const data = await res.json();
        if (!res.ok || !data.success) {
          throw new Error(data.error || `Failed to load security (${res.status})`);
        }
  ...
  return (
    <div className="min-h-screen bg-[#fcfaf6] text-[#172033]">
      <Modal open={!!active} onClose={() => setActive(null)} title={active?.title || 'Security'}>
        {active?.image && (
        ...
        {Array.isArray(active?.details) && active.details.length > 0 && (
          <div className="mt-5 space-y-3">
            <p className="text-xs font-black uppercase tracking-[0.22em] text-slate-500">Safety Measures & Guidelines</p>
      ...
      <header className="border-b border-[#d9d0c3] bg-white/90 backdrop-blur-xl sticky top-0 z-30">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-3 flex items-center justify-between">
          <Link href="/" className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-black tracking-tight">Security</p>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
        <div className="relative overflow-hidden rounded-[32px] border border-[#d9d0c3] bg-white shadow-[0_30px_90px_-70px_rgba(15,23,42,0.7)]">
          ...
          <div className="relative p-7 sm:p-10">
            ...
            <p className="mt-3 text-sm sm:text-base font-semibold text-slate-600 max-w-3xl leading-7">
              Security information, safety measures, and important guidelines for students and parents.
            </p>
        ...
        {loading ? (
          ...
          <div className="mt-10 grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-5">
            {items.map((item) => (
              <SecurityCard key={item.id} item={item} onView={() => setActive(item)} />
            ))}
            {!items.length && !error && (
