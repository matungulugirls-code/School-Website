...
function ModernLoadingSpinner({ message = "Loading achievements...", size = "medium" }) {
  ...
  return (
    <div className="fixed inset-0 bg-gradient-to-br from-gray-50 via-amber-50/30 to-yellow-50/20 flex items-center justify-center z-50">
      <div className="text-center">
        <div className="relative inline-block">
          ...
          <div className="absolute inset-0 flex items-center justify-center">
            <div className="bg-gradient-to-r from-amber-500 to-yellow-600 rounded-full animate-ping opacity-25"
    ...
    <div className="space-y-3">
      ...
      <div className="relative">
        <input
          ...
          className="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:ring-2 focus:ring-amber-500 focus:border-amber-500 transition-all duration-300 bg-white text-sm font-bold"
        />
      ...
      {tags.length > 0 && (
        <div className="flex flex-wrap gap-2 mt-2">
          {tags.map((tag, index) => (
            <div
              key={index}
              className="inline-flex items-center gap-1 bg-gradient-to-r from-amber-50 to-amber-100 text-amber-700 px-3 py-2 rounded-lg border border-amber-200 text-sm font-bold"
            >
              {tag}
              <button
                type="button"
          ...
          {images.map((image, index) => (
            <div key={index} className="relative group">
              ...
              <input
                ...
                placeholder="Caption"
                className="w-full mt-1 px-2 py-1 text-xs border border-gray-200 rounded focus:ring-1 focus:ring-amber-500"
              />
            </div>
          ))}
        </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="flex gap-3 pt-4 border-t border-gray-200">
              ...
              <button
                ...
                className="flex-1 bg-gradient-to-r from-amber-600 to-yellow-600 text-white px-4 py-3 rounded-xl hover:from-amber-700 hover:to-yellow-700 transition font-bold disabled:opacity-50"
              >
                {actionLoading ? (
        ...
        <div className="p-6">
          ...
          <div className="flex gap-3">
            ...
            <button
              onClick={onConfirm}
              disabled={loading}
              className="flex-1 bg-gradient-to-r from-red-600 to-orange-600 text-white px-4 py-3 rounded-xl hover:from-red-700 hover:to-orange-700 transition font-bold disabled:opacity-50"
            >
              {loading ? (
                <span className="flex items-center justify-center gap-2">
                  <CircularProgress size={16} className="text-white" />
...
export default function AchievementsPage() {
  ...
  const categoryColors = {
    ...
    Leadership: 'from-amber-600 to-orange-600',
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-amber-50 to-yellow-50 p-4 md:p-6">
      <Toaster position="top-right" richColors />
      
      {/* Header */}
      <div className="bg-gradient-to-br from-amber-600 via-orange-600 to-yellow-600 rounded-3xl shadow-xl p-6 md:p-8 mb-8">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            ...
            <p className="text-amber-100">Celebrating our school's excellence and success stories</p>
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <button
              ...
              className="bg-white text-amber-700 px-6 py-3 rounded-xl hover:bg-amber-50 transition font-bold flex items-center gap-2 shadow-lg"
            >
              <FaPlus />
              Add Achievement
            </button>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mt-6">
          <div className="bg-white/15 backdrop-blur-sm rounded-xl p-4">
            <p className="text-amber-200 text-sm">Total Achievements</p>
            <p className="text-white text-3xl font-black">{totalAchievements}</p>
          </div>
          
          {schoolStats?.meanScore && (
            <div className="bg-white/15 backdrop-blur-sm rounded-xl p-4">
              <p className="text-amber-200 text-sm">Current Mean Score</p>
              <p className="text-white text-3xl font-black">{schoolStats.meanScore}</p>
            </div>
          )}
          
          {schoolStats?.targetMean && (
            <div className="bg-white/15 backdrop-blur-sm rounded-xl p-4">
              <p className="text-amber-200 text-sm">Target Mean</p>
              <p className="text-white text-3xl font-black">{schoolStats.targetMean}</p>
            </div>
          )}
        </div>
      </div>
      
      {/* Mean Comparison Card */}
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {schoolStats.lastYearMean && (
              <div className="bg-gradient-to-br from-blue-50 to-blue-100 rounded-xl p-4">
                ...
                <p className="text-3xl font-black text-blue-700">{schoolStats.lastYearMean}</p>
              </div>
            )}
            ...
            {schoolStats.meanScore && (
              <div className="bg-gradient-to-br from-emerald-50 to-emerald-100 rounded-xl p-4">
                ...
                <p className="text-3xl font-black text-emerald-700">{schoolStats.meanScore}</p>
                {schoolStats.lastYearMean && (
                  <p className={`text-sm font-bold mt-1 ${schoolStats.meanScore > schoolStats.lastYearMean ? 'text-green-600' : 'text-red-600'}`}>
                    {schoolStats.meanScore > schoolStats.lastYearMean ? '↑' : '↓'} 
                    {(schoolStats.meanScore - schoolStats.lastYearMean).toFixed(2)} from last year
                  </p>
                )}
              </div>
            )}
            ...
            {schoolStats.targetMean && (
              <div className="bg-gradient-to-br from-amber-50 to-amber-100 rounded-xl p-4">
                ...
                <p className="text-3xl font-black text-amber-700">{schoolStats.targetMean}</p>
                {schoolStats.meanScore && (
                      ...
                      >
                        ...
                        <div className="flex gap-2 mt-4 pt-3 border-t border-gray-100">
                          <button
                            ...
                            className="flex-1 flex items-center justify-center gap-1 px-3 py-2 bg-amber-50 text-amber-700 rounded-lg hover:bg-amber-100 transition text-sm font-bold"
                          >
                            <FaEdit className="text-xs" /> Edit
                          </button>
                          <button
                            onClick={() => handleDeleteClick(achievement.id, achievement.title)}
                            className="flex-1 flex items-center justify-center gap-1 px-3 py-2 bg-red-50 text-red-700 rounded-lg hover:bg-red-100 transition text-sm font-bold"
                          >
      ...
      {totalAchievements === 0 && (
        <div className="bg-white rounded-3xl shadow-xl p-12 text-center">
          <FaTrophy className="text-6xl text-amber-300 mx-auto mb-4" />
          ...
          <button
            ...
            className="bg-gradient-to-r from-amber-600 to-yellow-600 text-white px-6 py-3 rounded-xl hover:from-amber-700 hover:to-yellow-700 transition font-bold inline-flex items-center gap-2"
          >
